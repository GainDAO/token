...
const doExecuteTest = (theSettings) => () => {
  ...
  const setupContracts = async (settings, startdistribution = false) => {
    ...
    try {
      paymenttoken = await setupPaymentToken(deployer, user1, user2, user3, rejecteduser, theSettings.paymentTokenVolume, theSettings.paymentTokenName)
      gaintoken = await setupGainDAOToken(deployer, theSettings.gainTokenname, theSettings.gainTokensymbol, theSettings.cDistVolumeWei)
      ...
      if (startdistribution) {
        await gaintoken.connect(deployer).mint(distribution.address, settings.cDistVolumeWei);
    ...
    for (let idx = 0; idx < buyCycles.length; idx++) {
      ...
      it(`${name} - is unable to buy above current rate`, async () => {
        ...
        try {
          ...
          const result = await userBuysGainTokens(paymenttoken, distribution, cycle.tokens_wei, rateundivided.sub("1"), user, proof, validto);
      ...
      it(`${name} - is able to buy at or above current rate`, async () => {
        ...
        const result = await userBuysGainTokens(paymenttoken, distribution, cycle.tokens_wei, ratewithslippage, user, proof, validto);
      ...
      it(`${name} - end ERC20 balance is correct`, async () => {
        let paymenttokenbalance = await paymenttoken.balanceOf(distribution.address);
        expect(paymenttokenbalance).to.equal(cycle.pool_balance_end_wei);
      });

      it(`${name} - transaction with invalid KYC proof fails`, async () => {
        const result = await userBuysGainTokens(paymenttoken, distribution, cycle.tokens_wei, cycle.rateundivided, user3, user1kycproof, validto);
        expect(result).to.equal(false)
        // expect(errorresult).to.be.revertedWith("KYC: invalid token");
      })

      it(`${name} - transaction with expired KYC proof fails`, async () => {
        const result = await userBuysGainTokens(paymenttoken, distribution, cycle.tokens_wei, cycle.rateundivided, user1, user1expiredproof, expired);
        expect(result).to.equal(false)
  ...
  describe("ERC20Distribution - Various tests related to distribution contract creation", () => {
    ...
    it("edge cases", async () => {
      ...
      // added to achieve 100% code coverage
      expect(await paymenttoken.decimals())
        .to.be.equal(18);
  ...
  describe("ERC20Distribution - Various tests related to distribution start", () => {
    ...
    it("detects distribution start correctly", async () => {
      ...
      const txmint = await token.connect(deployer).mint(distribution.address, theSettings.cDistVolumeWei);
  ...
  describe("ERC20Distribution - Various tests related to distribution end", () => {
    ...
    it("has expected exchange rate after distribution end", async () => {
      ...
      const buyratecalculated = theSettings.cDistStartRate.add(theSettings.cDistEndRate.sub(theSettings.cDistStartRate).div(two));
      ...
      const txapprove = await paymenttoken.connect(user1).approve(distribution.address, fiat_value);
  ...
  describe("ERC20Distribution - Distribute SIMUSD", () => {
    ...
    it("has funded the test accounts with USD", async () => {
      expect(await paymenttoken.balanceOf(user1.address))
        .to.equal(ethers.utils.parseEther("42000000001"), "user 1 funding failed");
      expect(await paymenttoken.balanceOf(user2.address))
        .to.equal(ethers.utils.parseEther("42000000002"), "user 2 funding failed");
      expect(await paymenttoken.balanceOf(user3.address))
  ...
  describe("ERC20Distribution - Purchase " + theSettings.gainTokenname, () => {
    ...
    beforeEach(async () => {
      try {
        ...
        const txmint = await gaintoken.connect(deployer).mint(distribution.address, theSettings.cDistVolumeWei);
        await waitForTxToComplete(txmint);
        ...
        console.error(`ERC20Distribution - Purchase ${theSettings.gainTokenname} - beforeEach - error ${ex.message}`);
      }
    });
    ...
    it("user 1 can purchase " + theSettings.gainTokenname, async () => {
      ...
      const tx1 = await paymenttoken.connect(user1).approve(distribution.address, fiat_value_insufficient);
      ...
      const tx2 = await paymenttoken.connect(user1).approve(distribution.address, fiat_value);
      ...
      const dist_balance_token = await paymenttoken.balanceOf(distribution.address);
    ...
    it("rejected user cannot purchase uGAIN", async () => {
      ...
      const tx1 = await paymenttoken.connect(rejecteduser).approve(distribution.address, fiat_value);
      await waitForTxToComplete(tx1);
  ...
  describe("ERC20Distribution - KYC", () => {
    ...
    it("contract owner can buy tokens with no approver set", async () => {
      ...
      const tx = await token.connect(deployer).mint(distribution.address, theSettings.cDistVolumeWei);
      await waitForTxToComplete(tx);
      ...
      await userBuysGainTokens(paymenttoken, distribution, buyamountwei, undefined, deployer, dummyproof, 0);
    ...
    it("is not allowed to buy with no kyc approver set", async () => {
      ...
      const tx = await gaintoken.connect(deployer).mint(distribution.address, theSettings.cDistVolumeWei);
      await waitForTxToComplete(tx);
  ...
  describe("ERC20Distribution - Token distribution", () => {
    ...
    it("initialization - has correct start rate", async () => {
      expect(await distribution.startrate_distribution()).to.equal(theSettings.cDistStartRate);
    })

    it("initialization - has correct end rate", async () => {
      expect(await distribution.endrate_distribution()).to.equal(theSettings.cDistEndRate);
    })

    it("initialization - has correct rate divider", async () => {
      expect(await distribution.dividerrate_distribution()).to.equal(theSettings.cDistDividerRate);
    })

    it("initialization - has correct total distribution volume", async () => {
      expect(await distribution.total_distribution_balance()).to.equal(theSettings.cDistVolumeWei);
    })
    ...
    it("start distribution - it can receive tokens for distribution", async () => {
      ...
      await gaintoken.connect(deployer).mint(distribution.address, theSettings.cDistVolumeWei);
    })
    ...
    it("start distribution - it cannot purchase tokens while paused", async () => {
      ...
      const result = await userBuysGainTokens(paymenttoken, distribution, buyamountwei, undefined, user1, user1kycproof, validto);
      expect(result).to.equal(false);
    })
  ...
  describe("ERC20Distribution - Slippage", () => {
    ...
    it('it is possible to buy at the current rate', async () => {
      ...
      await userBuysGainTokens(paymenttoken, distribution, buyamountwei, undefined, user1, user1kycproof, validto);
    ...
    it('it is possible to buy at a worse rate than current rate', async () => {
      ...
      await userBuysGainTokens(paymenttoken, distribution, startvolumewei, undefined, user1, user1kycproof, validto);
      ...
      expect(nextrateundivided).to.equal(calculateRateUndivided(theSettings, "150000", nextamount));

      // calculate a worse rate
      let worserate = nextrateundivided.add("10000")
      await userBuysGainTokens(paymenttoken, distribution, nextamountwei, worserate, user1, user1kycproof, validto);

      nextrateundivided = await distribution.currentRateUndivided(nextamountwei);
      expect(nextrateundivided).to.equal(calculateRateUndivided(theSettings, "151000", nextamount));
    ...
    it('it is not possible to buy when the rate has worsened', async () => {
      ...
      await userBuysGainTokens(paymenttoken, distribution, dummyvolumewei, undefined, user2, user2kycproof, validto);

      // attempt to buy at original rate 
      const result = await userBuysGainTokens(paymenttoken, distribution, nextamountwei, nextrateundivided, user1, user1kycproof, validto);
      expect(result).to.equal(false);
    });
  });
