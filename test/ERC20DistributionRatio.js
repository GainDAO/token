...
const executesRatiosTest = (theSettings) => () => {
  ...
  const setupContracts = async (settings, doInit = true) => {
    ...
    await gaintoken.grantRole(
      await gaintoken.DEFAULT_ADMIN_ROLE(),
      treasury.address
    );
    await gaintoken.grantRole(await gaintoken.MINTER_ROLE(), treasury.address);

    await distribution.grantRole(
      await distribution.DEFAULT_ADMIN_ROLE(),
      treasury.address
    );
    await distribution.grantRole(
  ...
  const distVolume = ethers.utils.formatUnits(
  ...
  testcases.push({
    offset: (distVolume - purchaseVolume).toString(),
    gain: purchaseVolume.toString(),
    rateOffset,
  });
  ...
  context("ratios", async () => {
    ...
    testcases
      .forEach((testcase) => {
        it(
          "tests ratio case " + testcase.offset + "/" + testcase.gain,
          async () => {
            const currentdistvolume =
              await distribution.current_distributed_balance();
            const offsetwei = ethers.utils.parseUnits(
              testcase.offset,
            ...
            if (currentdistvolume < offsetwei) {
              ...
              let currentrateundivided = ethers.BigNumber.from(
                await distribution.currentRateUndivided(deltawei)
              );
              ...
              expect(
                ...
                "user 1 bought enough ugain"
              ).to.equal(deltawei);
              expect(
                await distribution.current_distributed_balance(),
                "distribution at correct offset"
              ).to.equal(offsetwei);

              // clear offset balance from distribution contract
              const usdcbalance2 = await paymenttoken.balanceOf(
                distribution.address
              );
              const tx2 = await distribution.connect(pool).claimFiatToken();
              await waitForTxToComplete(tx2);
              expect(
                await paymenttoken.balanceOf(pool.address),
                "pool claims payment token offset"
              ).to.equal(usdcbalance2);
              expect(
                await paymenttoken.balanceOf(distribution.address),
                "distribution has no balance after claim"
              ).to.equal(0);
            }

            const currentrate = await distribution.currentRateUndivided(
            ...
            const actualratio =
              currentrate / (await distribution.dividerrate_distribution());
            ...
            const gainwei = ethers.utils.parseUnits(
              testcase.gain,
              await paymenttoken.decimals()
            );
            await userBuysGainTokens(
            ...
            expect(
              await gaintoken.balanceOf(user2.address),
              "user 2 bought enough ugain"
            ).to.equal(gainwei);
            const newdistvolume =
              await distribution.current_distributed_balance();

            // claim payment token tokens to the treasury
            const usdcclaim = await paymenttoken.balanceOf(
              distribution.address
            );
            const poolbalance = await paymenttoken.balanceOf(pool.address);
            // console.log("dist payment token claim is %s", ethers.utils.formatUnits(usdcclaim, theSettings.paymentTokenDecimals))
            const tx = await distribution.connect(pool).claimFiatToken();
            await waitForTxToComplete(tx);

            expect(
              await paymenttoken.balanceOf(pool.address),
              "pool claims payment token"
            ).to.equal(usdcclaim.add(poolbalance));
            expect(
              await paymenttoken.balanceOf(distribution.address),
              "no payment token left in distribution"
            ).to.equal(0);
